...
if(strcmp(benchmark_data, 'MPI'))
    ...
    gt = load('../dataset/MPI/mpii_human_pose_v1_u12_1/mpii_human_pose_v1_u12_1.mat'); % mat file gt with structures 
    RELEASE_img_index = find(~gt.RELEASE.img_train);    % all the indices of the non-train data
    ...
    target_dist = 41/35; % fit training  what is the target_dist 
...
center_map = produceCenterLabelMap([boxsize boxsize], boxsize/2, boxsize/2, model.sigma);   % person more likely to be in the center
...
if(strcmp(benchmark_data, 'MPI'))
    for i = 1:length(RELEASE_img_index)
        ...
        for person_index = 1:length(gt.RELEASE.annolist(img_index).annorect) 
            % seems multiple persons in the images. 
            count = count + 1;
            ...
            try
                oriImg = imread(imagePath); % read in 
            catch
                error('image cannot be loaded, make sure you have %s', imagePath);
            end

            try
                objpos = gt.RELEASE.annolist(img_index).annorect(person_index).objpos; % many joint pos? 
                center = [objpos.x objpos.y];   % labeld center center position
                scale_provided = gt.RELEASE.annolist(img_index).annorect(person_index).scale;
                scale0 = target_dist/scale_provided;    % adjust the scale of original one.
            catch
            ...
            score = cell(1, length(multiplier));    % 1x nScale
            pad = cell(1, length(multiplier));  % 1x nScale 

            for m = 1:length(multiplier)    % search all the scales 
                scale = scale0 * multiplier(m); % rescale to one size. 
                ...
                score{m} = applyDNN(imageToTest, net);  % all the scores in m cells 
                ...
                score{m} = imresize(score{m}, [size(oriImg,2) size(oriImg,1)]); % to original image coordinates. 
            end
            ...
            final_score = permute(final_score, [2 1 3]);  % back to matlab 

            % ----- generate prediction -----
            prediction = zeros(np,2);   % npx2 
            for j = 1:np
                [prediction(j,2), prediction(j,1)] = findMaximum(final_score(:,:,j));   % largest scores
            end
            prediction(order_to_standard,:) = prediction;   % permute the scores to MPII order 
            % prediction x,y position. 
            ...
            for j = 1:np
                ...
                % put the predicted value to the RELEASE structure
            end
        end
    end
    prediction_file = sprintf('predicts/MPI_prediction_model_%s.mat', model.description_short);
    save(prediction_file, 'RELEASE_predicted'); % only save no comparision
...
elseif(strcmp(benchmark_data, 'LSP'))
    ...
    for i = 1:testLength
        ...
        joint_gt(:,:,i) = gt.joints(1:2,:,1000+i)'; % only x,y from 1000 later 
        ...
        for j = 1:np
            if(makeFigure)
                max_value = max(max(final_score(:,:,j)));   % 16 channel max, max of image 
                imToShow = single(oriImg)/255 * 0.5 + mat2im(final_score(:,:,j), jet(100), [0 max_value])/2; % 1/2 original, 1/2 the scores
                imToShow = insertShape(imToShow, 'FilledCircle', [prediction(j,:) 2], 'Color', 'w');    % x,y, radius
                imToShow = insertShape(imToShow, 'FilledCircle', [joint_gt(j,1:2,i) 2], 'Color', 'g');  % i testLength
                imToShow = insertShape(imToShow, 'FilledRectangle', [center 3 3], 'Color', 'c');    % label the centers
            ...
            error_dist = norm(prediction(j,:) - joint_gt(j,1:2,i)); % distance, 
    ...
    save(prediction_file, 'prediction_all');    % save prediction result  
...
function bodysize = util_get_bodysize_size(rect)
    bodysize = norm(rect(10,:) - rect(3,:)); % following evalLSP_official left shoulder right hip
